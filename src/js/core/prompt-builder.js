import {
  ...
  DEFAULT_STATE,
  INDOOR_ENVIRONMENTS,
  OUTPUT_MODE_DIRECTIONS
} from "../config/default-state.js";
...
const FACING_LABELS = Object.freeze({
...
const safeLabel = (dictionary, key, lang) => dictionary[key]?.[lang] ?? String(key);
...
function pixelDirection(mode, lang) {
  const entries = {
    classicHd: {
      de:
...
function outlineDirection(mode, lang) {
  const entries = {
    dark: {
...
function paletteDirection(profile, mode, lang) {
  const entries = {
    natural: {
...
function colorBudgetDirection(value, lang) {
  if (value === "unlimited") {
...
function cameraDirection(state, lang) {
  const perspective = {
    topdown: {
...
function resolveLighting(state, profile, lang) {
...
function shadowDirection(state, lang) {
  const entries = {
    automatic: {
      de:
...
function backgroundDirection(state, lang) {
...
function outputDirection(state, lang) {
...
function resolveFrameSize(state) {
  if (state.frameSize !== "auto") {
    const parsed = Number.parseInt(state.frameSize, 10);
    if (Number.isFinite(parsed) && parsed > 0) return parsed;
  }

  if (CHARACTER_ASSET_TYPES.has(state.assetType)) return 128;
...
function resolveLayout(state) {
  if (state.sheetLayout !== "auto") {
    ...
    if (Number.isFinite(columns) && Number.isFinite(rows)) return { columns, rows };
  }
...
function scaleDirection(state, lang) {
...
function qualityRules(state, lang) {
  const rules = [];
...
function mainPrompt(state, profile, lang) {
...
function negativePrompt(state, profile, lang) {
...
function technicalSpecification(state, profile, lang) {
  const frame = resolveFrameSize(state);
  const layout = resolveLayout(state);
  const canvasWidth = frame * layout.columns;
  const canvasHeight = frame * layout.rows;
  ...
  if (lang === "de") {
    const lines = [
      `PROFIL: ${PROFILE_LABELS[profile].de}`,
  ...
  const lines = [
    `PROFILE: ${PROFILE_LABELS[profile].en}`,
...
export function buildProjectOutputs(input = {}) {
  ...
  for (const profile of requestedProfiles(state)) {
    for (const language of requestedLanguages(state)) {
      ...
      outputs.push({
        id: `${profile}-${language}`,
        profile,
        profileLabel: PROFILE_LABELS[profile][language],
        language,
...
export function validateState(input = {}) {
  ...
  if (CHARACTER_ASSET_TYPES.has(state.assetType) && characterHeight < 16) {
    errors.push("Die Figurenhöhe muss mindestens 16 Pixel betragen.");
  }
  if (CHARACTER_ASSET_TYPES.has(state.assetType) && characterHeight + padding * 2 > frame) {
  ...
  if (state.outputMode === "directional8" && state.sheetLayout !== "4x2" && state.sheetLayout !== "8x1" && state.sheetLayout !== "2x4" && state.sheetLayout !== "auto") {
...
export function getResolvedMetrics(input = {}) {
  const state = mergeState(input);
  const frame = resolveFrameSize(state);
  const layout = resolveLayout(state);
  return {
    ...
    directionCount: OUTPUT_MODE_DIRECTIONS[state.outputMode] ?? 1
  };
}
